...
public class GrowthcraftDecoMissingMappings {
    ...
    public static void missingMappingEventHandler(MissingMappingsEvent event) {
        event.getAllMappings(ForgeRegistries.Keys.ITEMS).forEach(missingItemMapping -> {
            if (missingItemMapping.getKey().getNamespace().equals(Reference.MODID)) {
                switch (missingItemMapping.getKey().toString()) {
                    case "growthcraft_deco:stairs_deepslate_brick_glowing" ->
                    ...
                    default ->
                            throw new IllegalStateException("Unexpected value: " + missingItemMapping.getKey().toString());
                }
            }
        });

        event.getAllMappings(ForgeRegistries.Keys.BLOCKS).forEach(missingBlockMapping -> {
            if (missingBlockMapping.getKey().getNamespace().equals(Reference.MODID)) {
                switch (missingBlockMapping.getKey().toString()) {
                    ...
                    default ->
                            throw new IllegalStateException("Unexpected value: " + missingBlockMapping.getKey().toString());
                }
            }
        });
    }
